...
public class DvrRevisionTests
{
    ...
    public void TwoEntriesReadFromTheSameFilesCompareAsTheSame()
    {
        ...
        var first = DvrEntryMapper.FromMessage(WithFiles(Entry(1, "completed", title: "Tatort"), File(Opened, Closed)))!;
        var second = DvrEntryMapper.FromMessage(WithFiles(Entry(1, "completed", title: "Tatort"), File(Opened, Closed)))!;

        Assert.NotSame(first.Files, second.Files);
        Assert.True(first.HasSameContentAs(second));

        var different = DvrEntryMapper.FromMessage(WithFiles(Entry(1, "completed", title: "Tatort"), File(Opened, Closed.AddSeconds(1))))!;
    ...
    public void AnEntryWithNoFilesComparesAsTheSameAsAnotherWithNone()
    {
        ...
        var first = DvrEntryMapper.FromMessage(Entry(1, "scheduled", title: "Tatort"))!;
        var second = DvrEntryMapper.FromMessage(Entry(1, "scheduled", title: "Tatort"))!;
