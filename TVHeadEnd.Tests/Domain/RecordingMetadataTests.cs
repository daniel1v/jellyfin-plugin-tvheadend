...
public class RecordingMetadataTests
{
    ...
    {
        var entry = DvrEntry.FromMessage(Message(1, contentType: DetectiveFilm))!;
    ...
    public void AnUpdateThatDoesNotMentionTheContentTypeKeepsIt()
    {
        ...
        var scheduled = DvrEntry.FromMessage(Message(1, contentType: Football))!;

        var update = Message(1);
        update.Set("state", "recording");

        var merged = DvrEntry.Merge(scheduled, DvrEntry.FromMessage(update)!, update);
    ...
    public void ARecordingIsClassifiedTheWayAGuideProgrammeIs()
    {
        ...
        var entry = DvrEntry.FromMessage(Message(1, state: "completed", contentType: DetectiveFilm))!;
    ...
    {
        var entry = DvrEntry.FromMessage(Message(1, state: "completed", contentType: Football))!;
        var recording = JellyfinDvrMapper.ToRecording(entry);
    ...
    {
        var entry = DvrEntry.FromMessage(Message(1, state: "completed"))!;
        var recording = JellyfinDvrMapper.ToRecording(entry);
    ...
    public void TheRecordingGroupsAreFilledByTheseVeryFlags(int contentType, string group)
    {
        ...
        var recording = JellyfinDvrMapper.ToRecording(
            DvrEntry.FromMessage(Message(1, state: "completed", contentType: contentType))!);
    ...
    {
        var scheduled = DvrEntry.FromMessage(Message(1, image: "imagecache/42", fanart: "imagecache/43"))!;
    ...
    public void AMapperThatOnlyReadsOneDvrEntryCannotKnowWhatItsChannelCarries()
    {
        ...
        // catalog is in reach.
        var recording = JellyfinDvrMapper.ToRecording(
            DvrEntry.FromMessage(Message(1, state: "completed"))!);
