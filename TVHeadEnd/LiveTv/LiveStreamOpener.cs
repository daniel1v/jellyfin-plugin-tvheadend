...
public sealed class LiveStreamOpener
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var description = programMap is null ? null : LiveStreamDescription.FromProgramMap(programMap);
    ...
    {
        if (description is not { IsUsable: true })
        {
            _logger.LogInformation(
                "Live TV: channel {ChannelId} ({ChannelName}) is playing, but its program map named no video stream, "
                + "so Jellyfin will inspect the stream itself. Program map: {ProgramMap}",
                stream.ChannelId,
        ...
        var video = description.Streams.First(media => media.Type == MediaStreamType.Video);
