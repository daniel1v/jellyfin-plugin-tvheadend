...
/// started and forgotten: <see cref="DisposeAsync"/> closes the input so FFmpeg can flush, awaits
/// every task it started, and kills the process if it has not gone by then. An earlier version
/// started the output pump and the error reader as discarded tasks, which meant a failure in
/// either surfaced nowhere and a cancelled stream could leave the process behind.
/// </para>
/// <para>
/// The process is always fed through a pipe rather than pointed at the source, so a channel that
/// is already being received does not cost a second TVHeadend subscription.
/// </para>
/// </remarks>
internal sealed class FfmpegSession : IAsyncDisposable
{
    ...
    public static FfmpegSession Start(
        string ffmpegPath,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!process.Start())
        {
            ...
            throw new InvalidOperationException(
                FormattableString.Invariant($"The FFmpeg process for {label} could not be started."));
        }

        logger.LogInformation("Live TV: FFmpeg started for {Label}, pid {ProcessId}", label, process.Id);

        var cancellation = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        return new FfmpegSession(process, cancellation, logger, label, writeOutput);
    }

    /// <summary>
    /// Closes the input so FFmpeg drains what it holds, without waiting for it to finish.
    /// </summary>
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        // Both are owned here, so both are awaited here. Neither throws: each swallows what its
        // own end of a dying pipe produces.
    ...
    private void ReportExit()
    {
        ...
        try
        {
            failed = _process.HasExited && _process.ExitCode != 0;
        }
        catch (InvalidOperationException)
        {
            failed = false;
        ...
        if (failed)
        {
            _logger.LogWarning("Live TV: FFmpeg for {Label} ended badly: {StderrTail}", _label, tail);
        }
        else
        {
            _logger.LogDebug("Live TV: FFmpeg for {Label} ended", _label);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // FFmpeg went away; the error tail says why.
        }
        catch (ObjectDisposedException)
        {
        ...
        {
            _logger.LogError(exception, "Live TV: carrying FFmpeg output for {Label} failed", _label);
        }
        finally
        {
