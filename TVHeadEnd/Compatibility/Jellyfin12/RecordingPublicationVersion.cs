...
{
    /// <summary>
    /// The date a recording is published with, which is a version marker rather than a fact.
    /// </summary>
    /// <remarks>
    /// <para>
    /// This exists entirely because of how Jellyfin's <c>ChannelManager</c> stores channel items.
    /// It rewrites one only when the modification date it is handed is later than the date it
    /// stored, and it compares nothing else -- so the only way an upgrade can reach a recording
    /// somebody already has is through that date. None of this describes a recording; all of it
    /// describes a persistence rule of one particular server.
    /// </para>
    /// <para>
    /// It never reads the clock. The same recording publishes the same date after a restart, on
    /// another machine, a year later -- which is what makes "rewritten exactly once" true rather
    /// than merely likely.
    /// </para>
    /// </remarks>
    public static class RecordingPublicationVersion
    {
        /// <summary>
