...
/// </summary>
public enum TransportStreamVerdict
{
    /// <summary>
...
/// </para>
/// </remarks>
public sealed class TransportStreamCheck
{
    private readonly byte[] _opening = new byte[TransportStreamDetector.ConclusiveLength];

    private int _collected;
    private TransportStreamVerdict _verdict = TransportStreamVerdict.Undecided;
    ...
    /// <see cref="TransportStreamDetector.ConclusiveLength"/> bytes have been seen. Once settled the
    ...
    public TransportStreamVerdict Accept(ReadOnlySpan<byte> chunk)
    {
        if (_verdict != TransportStreamVerdict.Undecided)
        {
            return _verdict;
        }
        ...
        if (TransportStreamDetector.IsTransportStream(_opening.AsSpan(0, _collected)))
        {
            _verdict = TransportStreamVerdict.TransportStream;
        }
        else if (_collected >= TransportStreamDetector.ConclusiveLength)
        {
            _verdict = TransportStreamVerdict.NotTransportStream;
        }
